...
if ($interactive) {
    ...
    if (-not $PSBoundParameters.ContainsKey('TargetRows')) {
        $answer=Read-Host 'Detail budget: 1500 compact, 10000 balanced, 50000 detailed (Enter for 10000)'
        if ($answer) {
            $parsed=0
...
$assessments=@(& "$PSScriptRoot/measure-snapshot-depth.ps1" -Path $roots -TargetRows $TargetRows -MaxProbeDepth $MaxProbeDepth -MaxEntries $MaxEntries -TimeBudgetSeconds $TimeBudgetSeconds -ExcludePath $OutDirRoot -Output '')
$depths=[ordered]@{}
...
$settings=[ordered]@{
    ...
    depthAssessment=[ordered]@{targetRows=$TargetRows; maxProbeDepth=$MaxProbeDepth; maxEntries=$MaxEntries; timeBudgetSeconds=$TimeBudgetSeconds; assessedAtUtc=[DateTime]::UtcNow.ToString('o'); results=$assessments}
}
